...
class ReactInertiaAgent(BaseAgent): 
    def __init__(self,
        ...
        # Normalize task_type: hierarchical/flat variants → base type for param_completion & tool description
        _TASK_TYPE_MAP = {
            'alfworld_hierarchical': 'alfworld',
            'alfworld_dqn': 'alfworld',
            'alfworld_rl_v1': 'alfworld',
            'alfworld_rl_v2': 'alfworld',
            'scienceworld_hierarchical': 'scienceworld',
            'scienceworld_dqn': 'scienceworld',
            'webshop_hierarchical': 'webshop',
            'webshop_flat': 'webshop',
            'pddl_hierarchical': 'pddl',
        }
        self.task_type = _TASK_TYPE_MAP.get(task_type, task_type)
        ...
        # --- Prompting 组件 ---
        self.check_actions_cmd = check_actions if isinstance(check_actions, str) and check_actions.strip() else "check valid actions"
        if isinstance(check_inventory, str) and check_inventory.strip():
            self.check_inventory_cmd = check_inventory
        elif isinstance(check_inventory, bool):
            self.check_inventory_cmd = "inventory" if check_inventory else self.check_actions_cmd
        else:
            self.check_inventory_cmd = "inventory"
        ...
        # [P0-2] Phase-conditioned prompt window（仅 HierInertia task 会动态写入此值）
        # None = 使用全量 memory（默认行为，与原始代码等价，baseline 不受影响）
        # 整数 N = LLM 调用时保留 system prompt + 最近 N 条消息
...
# # #                 print(f'[DEBUG INFO]: ScienceWorldParamCompletion is inited')
    ...
    def update(self, action='', state=''):
        ...
        # --- 解析 observation (legacy, guarded) ---
        if observation and self.use_parser and self.param_completion is not None and parsed_action_result:
            s_time = time.time()
...
# # #         print("[DEBUG INFO]: required_param_names is: ", required_param_names)
    ...
    def run(self, init_prompt_dict=None):
        ...
        # [P0-1] 移除原 30% 全局惯性比例硬上限（inertia_count/(steps+1) > 0.3）
        # 原硬上限会在执行阶段强制切回 LLM，与 HierInertia DQN 阈值策略冲突。
        # 现在仅由 inertia_max（连续惰性步上限）控制，配合 HierInertia 按 phase 动态调节。
        # 注意：此修改仅影响 use_inertia=True 代码路径，ReAct / AutoTool baseline 不受影响。
        if self.continuous_inertial_call_count >= self.inertia_max:
            inertial_flag = False
...
# # #                         if self.debug: print("[DEBUG INFO]: filled_params is: ", filled_params)
        ...
        if not execute_inertially:
            ...
            while thought_count_this_step < self.max_think_iters:
                ...
                # [P0-2] Phase-conditioned prompt window truncation
                # prompt_window 由 HierInertia task 按 phase 动态写入（None=全量，不截断）
                # EXECUTION phase → 短窗口减少 token；EXPLORATION/TRANSITION → 全量
                # baseline (ReAct/AutoTool) 不写入 prompt_window，此分支不会触发
                if self.prompt_window is not None and len(llm_input_messages) > self.prompt_window + 1:
                    # 始终保留 index=0 的 system prompt，取最近 prompt_window 条对话
                    llm_input_messages = [llm_input_messages[0]] + llm_input_messages[-self.prompt_window:]
