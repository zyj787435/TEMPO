...
def _get_agent_token_counts(agent):
    """Extract cumulative token counts from the agent's metadata."""
    try:
        meta = agent.current_sequence.get("metadata", {})
        return {
            "input_tokens": meta.get("total_input_token", 0),
            "output_tokens": meta.get("total_output_token", 0),
            "n_calls": meta.get("total_llm_calls", 0),
        }
    except:
...
class EvalPddl(BaseTask):
    ...
    def __init__(self,
        ...
        # llm may be None — agent uses autool for LLM calls
    ...
    def evaluate(self):
        ...
        for id in range(num_envs):
            ...
            tok = _get_agent_token_counts(self.agent)
            print(f"[TOKEN] Example {id}: in={tok['input_tokens']}, out={tok['output_tokens']}, calls={tok['n_calls']}")
            print(f"[TIMER] Example {id}: elapsed={elapsed:.2f}s")
