...
class Evalalfworld(BaseTask):
    def __init__(self,
        ...
        self._resume_data = {}
        _existing_txt = os.path.join(log_path, "alfworld.txt") if log_path else None
...
[SYSTEM]: Your previous action triggered "Nothing happens". Please REFLECT: Why did it fail? (e.g. wrong tool? closed container?). DO NOT repeat the same action. Try something else."""
    ...
    def evaluate(self):
        ...
        self.num_exams = 134  # ✅ [修正] 恢复全量测试 (134题)
        ...
        for id in range(self.num_exams):
            ...
            # Resume: skip already-completed examples but still advance env state above
            if id in _completed:
