"""
alfworld_dqn.py — AlfWorld task with DQN-based RL (no pre-trained weights).
Registers as task type "alfworld_dqn" to coexist with standard "alfworld".
"""
import numpy as np
import time
from .rl_module_dqn import AutoToolRLAgent

# Module-level DQN agent (fresh start, no rl_model.pth)
...
@registry.register_task("alfworld_dqn")
class EvalalfworldDQN(BaseTask):
    def __init__(self,
                 llm_config=None,
    ...
    def evaluate_env(self, index, ob='', examples=None):
        ...
        for i in range(self.max_num_steps):
            ...
            _global_rl_agent.store_transition(curr_state, rl_action, rl_r, next_state, done)
            _global_rl_agent.learn()
            ...
            if done:
                ...
                print(f"[INERTIA] Example {index}: steps={total_steps}, A0={a0_count}, A1={a1_count}, A2={a2_count}", flush=True)
