...
class ReactInertiaAgent(BaseAgent): 
    def __init__(self,
        ...
        # 
        _project_root = os.environ.get("PROJECT_PATH", os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
        self.log_file = os.path.join(_project_root, 'logs', 'trajectories', f"{self.task_type}_inertia_{inertia_threshold}_{inertia_alpha}_{time.strftime('%Y%m%d_%H%M%S')}.json")
        self.action_log = {
...
# # #                 print('f[DEBUG INFO]: check if tool graph is loaded successfully')
                ...
                self.param_dependency_path = os.path.join(_project_root, 'logs', 'param_dependency', self.task_type, f"param_dependency_{time.strftime('%Y%m%d_%H%M%S')}.json")
                print("")
            except Exception as e:
                print(f": {e}")
...
# # #                 print(f'[DEBUG INFO]: ScienceWorldParamCompletion is inited')
    ...
    # ScienceWorld action verb prefixes for fallback extraction
    _SW_ACTION_VERBS = (
        'go to ', 'look around', 'look at ', 'look in ', 'pick up ',
        'put down ', 'move ', 'use ', 'open ', 'close ', 'activate ',
        'deactivate ', 'focus on ', 'inventory', 'task', 'wait',
        'read ', 'mix ', 'pour ', 'eat ', 'check valid',
    )

    def _extract_action_fallback(self, response_str):
        """Fallback: extract action from unstructured LLM output (e.g. DeepSeek reasoning leak).
        Scans lines bottom-up for a known action verb, or extracts quoted commands.
        Returns the extracted action string, or None if not found.
        """
        lines = response_str.strip().split('\n')
        for line in reversed(lines):
            clean = line.strip().lstrip('*->#| ').strip()
            if any(clean.lower().startswith(v) for v in self._SW_ACTION_VERBS):
                action = clean.split('\n')[0].strip()
                if action:
                    print(f"[FALLBACK PARSE] Recovered action: '{action[:80]}'", flush=True)
                    return action
        # Check for quoted embedded actions
        m = re.search(
            r'["`\']((?:go to|look around|look at|look in|pick up|put down|move|use|open|close'
            r'|activate|deactivate|focus on|inventory|task|wait|read|mix|pour|eat|check valid)[^"`\']{0,80})["`\']',
            response_str, re.IGNORECASE)
        if m:
            print(f"[FALLBACK PARSE] Extracted from quote: '{m.group(1)[:80]}'", flush=True)
    ...
    def parse_response_str(self, response_str):
        ...
        if match:
            ...
            # Strip ** markdown prefix (DeepSeek sometimes outputs bold formatting)
            if action.startswith('**'):
                m2 = re.search(r'["\u2018\u2019`]([^"\'`]{3,50})["\u2018\u2019`]', action)
                action = m2.group(1).strip() if m2 else action.lstrip('*').strip()
            ...
            if think_only_match:
                 ...
                 if "Action:" not in response_str[think_only_match.end():]:
                     ...
                     fallback = self._extract_action_fallback(thought)
                     if fallback:
                ...
                fallback = self._extract_action_fallback(response_str)
                if fallback:
    ...
    def update(self, action='', state=''):
        ...
        # ---  observation --- (guarded duplicate removed)
# # #             # print(f'[DEBUG INFO]: parsed_observation_result is: {parsed_observation_result}')
        ...
        s_time = time.time()
        # ---  ---
        if parsed_action_result is None:
            return
        if parsed_action_result.get("tool_name") != "unknown":
            ...
            if self.round_llm_duration == 0.0:
                # Inertia call: stronger feedback
...
# # #         print("[DEBUG INFO]: required_param_names is: ", required_param_names)
    ...
    def run(self, init_prompt_dict=None):
        ...
        self.round_llm_duration = 0.0
        self.round_cost = 0.0
        rl_step_start = time.time()  # [LATENCY] RL overhead timer
        final_action = None
        ...
        if self.memory.get_memory(parse=True):
            ...
            if self.continue_invalid_tool_count >= 2:
                ...
                print(f"[LATENCY] step={self.steps} mode=Inertia(forced) t_llm=0.0ms t_rl={(time.time()-rl_step_start)*1000:.2f}ms")
...
# # #                         if self.debug: print("[DEBUG INFO]: filled_params is: ", filled_params)
                        ...
                        if filling_successful:
                            ...
                            self.token_counts["output_tokens"] = 0
                            #
                            print(f"[LATENCY] step={self.steps} mode=Inertia t_llm=0.0ms t_rl={(time.time()-rl_step_start)*1000:.2f}ms")
                            return (final_action is not None), final_action
                        else:
        ...
        _lat_mode = "Inertia" if execute_inertially else "LLM"
        print(f"[LATENCY] step={self.steps} mode={_lat_mode} t_llm={self.round_llm_duration*1000:.1f}ms t_rl={(time.time()-rl_step_start)*1000:.2f}ms")
        return (final_action is not None), final_action
