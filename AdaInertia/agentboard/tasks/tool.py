...
class EvalTool(BaseTask):
    def __init__(self,
        ...
        import re as _re
        self._resume_data = {}
        _task_name_for_resume = env_config.get("name", "tool-query")
        _existing_txt = os.path.join(log_path, f"{_task_name_for_resume}.txt") if log_path else None
        if _existing_txt and os.path.exists(_existing_txt):
            with open(_existing_txt) as _f:
                for _line in _f:
                    _m = _re.match(r'\[EXP\] (\d+): \[success_rate\]: (\w+), \[progress_rate\]: ([\d.]+), \[grounding_acc\]: ([\d.eE+\-]+), \[score_state\]: (.*)', _line)
                    if _m:
                        _idx = int(_m.group(1))
                        _sr = 1.0 if _m.group(2) == 'True' else 0.0
                        _pr = float(_m.group(3))
                        _ga = float(_m.group(4))
                        _ss_str = _m.group(5).strip()
                        try:
                            _ss = eval(_ss_str)
                        except Exception:
                            _ss = []
                        self._resume_data[_idx] = (_sr, _pr, _ga, _ss)
    ...
    def evaluate(self):
        ...
        # Respect num_exam limit from run config (e.g. to run only academia subset)
        num_exam = self.run_config.get("num_exam", dataset_size)
        dataset_size = min(dataset_size, num_exam)
        success_rates = []
        progress_rates = []
        grounding_scores = []
        score_state_records = []
        ...
        _completed = getattr(self, '_resume_data', {})
        if _completed:
            print(f"[RESUME] Will skip {len(_completed)} already-completed examples.", flush=True)

        for id in range(dataset_size):
            # Resume: skip already-completed examples
            if id in _completed:
                _sr, _pr, _ga, _ss = _completed[id]
                success_rates.append(int(_sr))
                progress_rates.append(_pr)
                grounding_scores.append(_ga)
                score_state_records.append(_ss)
                num_steps.append(0)
                _done_bool = (_sr == 1.0)
                with open(self.agentboard.log_summary_path, "a+") as _wf:
                    _wf.write(f"[EXP] {id}: [success_rate]: {_done_bool}, [progress_rate]: {_pr}, [grounding_acc]: {_ga}, [score_state]: {_ss} \n")
                continue
