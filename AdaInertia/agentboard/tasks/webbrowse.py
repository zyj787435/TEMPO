...
URL: {url}"""
    ...
    def evaluate(self):
        ...
        grounding_acc_avg = []
        score_state_avg = []
        # print(f'[debug] test_file_list: {self.test_file_list}')
        for idx, config_file in enumerate(self.test_file_list):
            ...
            try:
                ...
                # print(f"[debug] task_id: {task_id}, success: {success}, progress_score: {progress_score}, grounding_acc: {grounding_acc}, score_state: {score_state}, steps: {steps}")
                ...
                if self.env_config["save_trace_enabled"]:
                    ...
                    # )
                    self.env.save_trace(
                        Path(self.result_dir) / "traces" / f"unknown.zip"
                    )
        ...
        if len(success_rate) > 0:
            ...
            sr = 0.0
            print("Warning: No tasks were evaluated. Setting success rate to 0.")
        if len(grounding_acc_avg) > 0:
            pr = sum(progress_scores) * 1.0 / len(progress_scores)
        else:
            pr = 0.0
            print("Warning: No tasks were evaluated. Setting progress rate to 0.")
        if len(score_state_avg) > 0:
            gr = sum(grounding_acc_avg) * 1.0 / len(grounding_acc_avg)
        else:
            gr = 0.0
            print("Warning: No tasks were evaluated. Setting grounding accuracy to 0.")
